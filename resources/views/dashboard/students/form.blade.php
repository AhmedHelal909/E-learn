...
<div class="col-md-6">
    <div class="form-group">
        ...
        <select class="form-control" id="exampleFormControlSelect1" name="classroom_id">
           
            @foreach ( App\Models\Classroom::get() as $classroom)
             <option {{ (isset($row) && $row->country_id == $classroom->id) ? 'selected style=color:red ': '' }} value="{{$classroom->id}}">{{$classroom->name}}</option>
            @endforeach
...
<div class="col-md-12">
    <div class="form-group">
        ...
        <select class="form-control" id="exampleFormControlSelect1" name="term_id">
            ...
            @foreach ( App\Models\Term::get() as $Term)
             <option {{ (isset($row) && $row->term_id == $Term->id) ? 'selected style=color:red ': '' }} value="{{$Term->id}}">{{$Term->name}}</option>
            @endforeach
