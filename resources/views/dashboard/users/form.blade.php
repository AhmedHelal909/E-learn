...
<div class="col-md-6">
    <div class="form-group">
        ...
        <select name="type" id="" class="form-control  @error('type') is-invalid  @enderror ">
            <option {{ (isset($row) && $row->type ==  2) ? 'selected style=color:red ': '' }} value="2">@lang('site.student')</option>
            <option {{ (isset($row) && $row->type ==  1) ? 'selected style=color:red ': '' }} value="1">@lang('site.admin')</option>
        </select>
        @error('type')
            <small class=" text text-danger" role="alert">
                <strong>{{ $message }}</strong>
